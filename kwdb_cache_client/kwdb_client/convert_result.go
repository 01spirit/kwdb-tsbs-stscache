...
func InsertEmptyBytes(singleSemanticSegment []string) []byte {
...
func ResponseToByteArrayWithParams(resp pgx.Rows, datatypes []string, tags []string, metric string, partialSegment string) []byte {
	result := make([]byte, 0)
	...
	nb, _ := Int64ToByteArray(int64(segIdx))
...
func InterfaceToByteArray(index int, datatype string, value interface{}) []byte {
	...
	case "float64":
		if value != nil {
			...
			} else if val, b := value.(pgtype.Numeric); b {
				valf, err := val.Float64Value()
				if err != nil {
					log.Fatal(err)
				}
				fBytes, err := Float64ToByteArray(valf.Float64)
				if err != nil {
					log.Fatal(fmt.Errorf(err.Error()))
				} else {
					result = append(result, fBytes...)
...
func ResponseToProto(respData [][]interface{}, datatypes []string, tags []string, metric string, partialSegment string, starSegment string) *codec.SemanticMeta {
	var numberOfTable int64 = 0

	dataBytes := make([]byte, 0)
	semSeries := make([]*codec.SemanticSeries, 0)
	...
	if len(respData) == 0 {
		for _, ss := range singleSegments {
			zero, _ := Int64ToByteArray(int64(0))
			semSeries = append(semSeries, &codec.SemanticSeries{
				SeriesSegment: ss,
				SegmentLen:    int64(len(ss)),
				Value:         zero,
			})
		}
		return &codec.SemanticMeta{
			MetaSegment: starSegment,
			SegmentLen:  int64(len(starSegment)),
			SeriesNum:   int64(len(singleSegments)),
			SeriesArr:   semSeries,
		}
	}
		...
		if tag != curTag {
			for segIdx < len(tags) {
				...
				if segTag != curTag {
					zero, _ := Int64ToByteArray(int64(0))
					semSeries = append(semSeries, &codec.SemanticSeries{
						SeriesSegment: singleSegments[segIdx],
						SegmentLen:    int64(len(singleSegments[segIdx])),
						Value:         zero,
					})
					numberOfTable++
					segIdx++
	...
	return &codec.SemanticMeta{
		MetaSegment: starSegment,
		SegmentLen:  int64(len(starSegment)),
		SeriesNum:   numberOfTable,
		SeriesArr:   semSeries,
	}
}

func ProtoToResponse(semMeta *codec.SemanticMeta, datatypes []string) ([][][]interface{}, int, []uint8, [][]int64, [][]string) {
	if semMeta == nil || semMeta.SeriesNum == 0 {
		return nil, 0, nil, nil, nil
	}
	...
	for _, series := range semMeta.SeriesArr {
		seprateSemanticSegments = append(seprateSemanticSegments, series.SeriesSegment)
		seriesLength = append(seriesLength, int64(len(series.Value)))

		curLen = int64(len(series.Value))
		index := 0
		length := len(series.Value)
			...
			for len(values) < lines {
				...
				for i, d := range datatypes {
					...
					case "string":
						...
						index += STRINGBYTELENGTH
						bEndIdx := index
						tmp := ByteArrayToString(series.Value[bStartIdx:bEndIdx])
						value = append(value, tmp)
						break
					case "bool":
						bStartIdx := index
						index += 1
						bEndIdx := index
						tmp, err := ByteArrayToBool(series.Value[bStartIdx:bEndIdx])
						if err != nil {
							log.Fatal(err)
						}
						value = append(value, tmp)
					...
					case "int64":
						...
						index += 8
						iEndIdx := index
						tmp, err := ByteArrayToInt64(series.Value[iStartIdx:iEndIdx])
						str := TimeInt64ToString(tmp)
						tm, err := time.Parse(goTimeFmt, str)
						if err != nil {
							log.Fatal(err)
					...
					case "float64":
						...
						index += 8
						fEndIdx := index
						tmp, err := ByteArrayToFloat64(series.Value[fStartIdx:fEndIdx])
						if err != nil {
							log.Fatal(err)
						}
						value = append(value, tmp)
					...
					default:
						...
						index += 8
						sEndIdx := index
						tmp, err := ByteArrayToFloat64(series.Value[sStartIdx:sEndIdx])
						if err != nil {
							log.Fatal(err)
						}
						value = append(value, tmp)
