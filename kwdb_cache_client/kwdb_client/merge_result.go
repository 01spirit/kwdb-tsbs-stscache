...
func RemainQuery(queryTemplate string, flagArr []uint8, timeRangeArr [][]int64, tagArr [][]string) (string, int64, int64) {
	...
	if num > 2 {
		aIdx := strings.Index(queryTemplate, "AND k_timestamp < '?'")
		oIdx := strings.Index(queryTemplate, "ORDER")
		predicate += " "
		predicate += queryTemplate[aIdx+len("AND k_timestamp < '?'")+1 : oIdx-1]
	...
	for i := 0; i < len(flagArr); i++ {
		if flagArr[i] == 1 {
			...
			splitQuery := fmt.Sprintf("(%s='%s' AND k_timestamp >= '%s' AND k_timestamp < '%s'%s)",
	...
	//qstr := fmt.Sprintf("k_timestamp >= '%s' AND k_timestamp < '%s' AND %s IN (%s) AND (%s)",
	//	TimeInt64ToString(minTime), TimeInt64ToString(maxTime), tagArr[0][0], strings.Join(tags, ","), strings.Join(queries, " OR "))
	//
	//queryTemplate = strings.Replace(queryTemplate, " AND k_timestamp >= '?'", "", -1)
	//queryTemplate = strings.Replace(queryTemplate, " AND k_timestamp < '?'", "", -1)
	//
	//queryTemplate = strings.Replace(queryTemplate, "?", qstr, 1)

	queryTemplate = strings.Replace(queryTemplate, "?", strings.Join(tags, ","), 1)
	queryTemplate = strings.Replace(queryTemplate, "?", TimeInt64ToString(minTime), 1)
	queryTemplate = strings.Replace(queryTemplate, "?", TimeInt64ToString(maxTime), 1)
